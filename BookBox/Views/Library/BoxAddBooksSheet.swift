...
/// 箱子加书面板 — 两种方式：拍照识别新书装入，或从当前书库的未归位书籍中挑选搬入
struct BoxAddBooksSheet: View {
    let boxId: Int
    let boxName: String
    let libraryId: Int?
        ...
        VStack(spacing: 0) {
            if isLoadingBooks {
                ...
                ContentUnavailableView {
                    ...
                    Text("本书库没有未归位的书籍。可先通过拍照识别录入，再装入箱子。")
                }
            } else {
                List {
                    Section {
    ...
    private func addSelectedBooks() {
        ...
        Task {
            ...
            do {
                _ = try await NetworkService.shared.addBooksToBox(
                    boxId: boxId,
                ...
                errorMessage = "装入箱子失败: \(error.chineseDescription)"
            }
        }
    }
}
